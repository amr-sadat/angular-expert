...
- Custom `ControlValueAccessor` implementations should invoke the `registerOnChange` callback inside the DOM event handler that captures user input. Deferring the call (`setTimeout`, debounced observable that has not yet emitted) leaves the parent `FormControl` out of sync in zoneless apps until the next CD trigger.
...
- Classic `@angular/animations` (`trigger`, `state`, `transition`, `animate`) is zone-dependent and deprecated in v20. In zoneless apps it may silently fail to advance multi-step animation sequences. Use `animate.enter` / `animate.leave` for element animations instead.
